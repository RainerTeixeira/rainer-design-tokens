...
describe('release-package.ts', () => {
  ...
  describe('🔍 Validação de Tokens', () => {
    it('deve validar todos os arquivos de tokens', () => {
      const result = execSync(
        'npx tsx scripts/release-package.ts --validate-only',
    ...
    it('deve falhar validação se arquivo essencial estiver faltando', () => {
      ...
      const backupPath = join(
        tokensDir,
        'primitives',
        'color-palette.json.backup'
      );

      if (existsSync(originalPath)) {
        // Usar operações de fs nativas para compatibilidade cross-platform
  ...
  describe('🔐 Configuração de Autenticação', () => {
    it('deve verificar configuração NPM', () => {
      ...
      const result = execSync(
        'npx tsx scripts/release-package.ts --setup-auth',
        { encoding: 'utf-8', stdio: 'pipe', cwd: rootDir }
      );
      ...
      try {
        ...
        const result = execSync(
          'npx tsx scripts/release-package.ts --setup-auth',
          {
            ...
            stdio: 'pipe',
            cwd: rootDir,
            env: { ...process.env, NPM_TOKEN: testToken },
          }
        );

        expect(result).toContain(
          'Configurando autenticação npm a partir de .env'
        );
      } finally {
        // Limpar
  ...
  describe('🌐 Configuração GitHub', () => {
    it('deve verificar configuração Git', () => {
      const result = execSync(
        'npx tsx scripts/release-package.ts --github-only',
        { encoding: 'utf-8', stdio: 'pipe', cwd: rootDir }
      );

      expect(result).toMatch(
        /(Git já está inicializado|Inicializando repositório Git)/
      );
    });
    ...
    it('deve verificar remote origin', () => {
      ...
      expect(result).toMatch(
        /(Remote 'origin' configurado|Remote 'origin' não configurado)/
      );
    });
  });
  ...
  describe('📝 Commit e Push', () => {
    it('deve verificar status do Git', () => {
      ...
      try {
        const result = execSync(
          'npx tsx scripts/release-package.ts --skip-npm --skip-github',
          { encoding: 'utf-8', stdio: 'pipe', timeout: 10000, cwd: rootDir }
        );
  ...
  describe('🚀 Publicação NPM', () => {
    it('deve suportar dry-run', () => {
      const result = execSync(
        'npx tsx scripts/release-package.ts --dry-run --skip-git',
        { encoding: 'utf-8', stdio: 'pipe', cwd: rootDir }
      );
  ...
  describe('📋 Criação de Release', () => {
    ...
    it('deve verificar GitHub CLI se disponível', () => {
      ...
      expect(result).toMatch(
        /(GitHub CLI não encontrado|Criando release no GitHub)/
      );
    });
  });
  ...
  describe('⚡ Performance e Erros', () => {
    ...
    it('deve tratar argumentos inválidos', () => {
      ...
      expect(() => {
        execSync(
          'npx tsx scripts/release-package.ts --opcao-inexistente --validate-only',
    ...
    it('deve mostrar help', () => {
      const result = execSync('npx tsx scripts/release-package.ts --help', {
        encoding: 'utf-8',
        stdio: 'pipe',
        cwd: rootDir,
      });
